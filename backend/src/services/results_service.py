...
def _datapackage_warning(camtrap_out) -> tuple[bool, list[str]]:
    """Check camtrap_dp_verified/datapackage.json for the auto-generation marker.

    Returns (generated, fabricated_fields) -- (False, []) when the file is
    missing, unreadable, or wasn't auto-generated by this app.
    """
    dp_path = camtrap_out / "datapackage.json"
    if not dp_path.exists():
        return False, []
    try:
        marker = json.loads(dp_path.read_text()).get("wildintelGenerated")
    except (OSError, ValueError):
        logger.exception("Failed to read %s", dp_path)
        return False, []
    if not marker:
        return False, []
    return True, marker.get("fabricatedFields", [])
...
def compute_results() -> dict:
    ...
    datapackage_generated, datapackage_fabricated_fields = _datapackage_warning(p["camtrap_out"])
    return {
        "session_dir":     str(session_service.session_dir()),
        "output_dir":      str(p["camtrap_out"]),
        "occupancy_dir":   str(p["occupancy_out"]),
        "datapackage_generated":         datapackage_generated,
        "datapackage_fabricated_fields": datapackage_fabricated_fields,
        "total":           len(periods),
